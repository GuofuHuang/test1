...
export class ExecutiveInventoryPage implements OnInit{
  ...
  ngOnInit() {
    ...
    this.data.todayMonthLastYear = moment(this.today).utc().startOf('day').subtract(1, 'years').format();
    this.data.endOfMonthLastYear = moment(this.today).utc().endOf('month').subtract(1, 'years').format();
    this.data.endOfLastYear = moment().utc().endOf('year').subtract(1, 'years').format();
    this.data.yearNumber = new Date().getFullYear();
    this.data.priorYearNumber = new Date().getFullYear() - 1;
    this.data.monthNumber = new Date().getMonth() + 1;
    this.init();
  ...
  async projectedFutureSales() {
    ...
    let lastYearMonthRangeEndOfYear = {
      gte: new Date(this.data.todayMonthLastYear),
      lte: new Date(this.data.endOfLastYear)
    }
